...
export function DepositWidget({
  ...
  const fdryMint = assetMint ?? VAULT_ASSET_MINT_STR;
  ...
  const depositBtnLabel =
    !pubkey ? "Connect wallet first"
      ...
      : depositOverBalance ? `Need more ${assetSym} first`
    ...
    <div className="rounded-2xl sm:rounded-3xl border border-line bg-white p-4 sm:p-6 md:p-8">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-5">
        ...
        {pubkey ? (
          ...
          <div className="fdry-wallet-scope w-full sm:w-auto">
            <WalletMultiButton className="!text-xs !px-3 !py-1.5 !h-auto !rounded-xl !bg-molten !text-white hover:!opacity-95 !font-sans !leading-none" />
          </div>
        )}
      </div>
      ...
      <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-1 sm:flex">
                {["100", "1000", "10000"].map((v) => (
                  <button
                    key={v}
                    onClick={() => setAmount(v)}
                    className="px-2 py-2 sm:py-1 text-xs rounded-md border border-line text-muted hover:bg-white transition"
                    type="button"
                  >
        ...
        {mode === "deposit" && isFdryDepositAsset && (
          <BuyFdryCta
            amount={amtValid ? amt : 0}
            balance={fdryBalance}
