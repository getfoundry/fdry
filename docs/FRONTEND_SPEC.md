...
```
/Users/lekt9/Projects/fdry/frontend/
├── index.html
├── vite.config.ts
├── tailwind.config.ts
├── tsconfig.json
...
2. User types FDRY amount. `DepositCapMeter` shows `maxDeposit = poolLiq * 0.01`
   (read from `meteora.getMeteoraPoolLiquidity(FDRY_SOL_POOL)`). Submit disabled
   if over cap or `poolLiq === 0`.
3. `SlippagePreview` shows Jupiter quote (FDRY → SOL, `slippageBps: 200`) with
   expected SOL out, price impact %, route hops.
4. User clicks **Deposit**. Confirmation sheet lists the two txs that will be
   requested in sequence: "1/2 Swap FDRY → SOL (Jupiter)" and "2/2 Buy vault
   tokens + lock deposits (Symmetry)".
5. After tx1 confirms, measure `solReceived = getBalanceChange(userWallet, SOL_MINT)`.
6. Build Symmetry `buyVaultTx` with the measured SOL, then `lockDepositsTx`.
...
7. Toast "Deposit queued. Keeper will execute within minutes; vault_token
...
- Creator fee is disabled at Symmetry protocol level; operator income from
  this lane is $0 until enabled by Symmetry governance.
