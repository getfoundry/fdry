...
│   └── daily_signal/
│       └── YYYY-MM-DD.json  # populated by bridge-harness emitter
...
### LAYER 1 — Signal  (producer: unify/.bridge-harness)

- **Producer path**: `[INTERNAL_PATH]/.bridge-harness/weights_emit.py`
- **Output path**: `~/Projects/fdry/runs/daily_signal/YYYY-MM-DD.json`
...
- **Path**: `~/Projects/fdry/ledger/YYYY-MM-DD.json`
...
|---|---|---|---|
| L1 Signal | **Existing bridge-harness host** (local/VPS already running the nightly loop) | Nightly cron already configured; adds the emitter as the final step. | JSON file in `runs/daily_signal/`. |
