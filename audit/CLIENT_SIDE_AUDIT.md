...
10. **`withdrawal_waiting_period=0`** (in current Ranger config) means
    instant withdraw works without a queue. If this changes, the UI
    must reflect it. Cross-check
    `scripts/lib/rangerConfig.ts:DEFAULT_VAULT_CONFIG`.
